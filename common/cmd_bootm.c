...
int do_bootzmg (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	ulong	addr;
	int	i,  verify;
	char	*s;
	...
	_do_bootzmg(cmdtp, flag, argc, argv, addr);
}

U_BOOT_CMD(
 	bootzmg,	CFG_MAXARGS,	1,	do_bootzmg,
 	"bootzmg   - boot zimage from memory\n",
 	""
);
